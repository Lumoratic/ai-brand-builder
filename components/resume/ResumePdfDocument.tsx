...
/** Minimum description space (pt) kept with an entry header on long entries. */
...
/**
 * Section title + first child are one pagination unit (wrap={false}).
 * minPresenceAhead on the title alone cannot bind it to content — react-pdf
 * paginates sibling nodes independently, which causes orphan headers.
 */
function PdfSection({
  title,
  children,
...
function PdfContactLine({ data }: { data: ResumeAssetData }) {
...
function PdfTimelineEntry({
  ...
  const header = (
    <>
      {primary ? <Text style={styles.entryTitle}>{primary}</Text> : null}
      {secondary ? (
            ...
            {certifications.map((entry: ResumeCertification, index) => (
              ...
              >
                {hasResumeText(entry.name) ? (
                  <Text style={styles.entryTitle}>{entry.name.trim()}</Text>
                ) : null}
                <PdfMetaLine
